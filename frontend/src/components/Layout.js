...
const Layout = ({ children }) => {
  ...
  const navItems = [
    ...
    { icon: CreditCard, label: 'All Payments', path: '/all-payments', show: user.role === 'Front Desk Executive' || user.role === 'Admin' },
    { icon: Clock, label: 'Pending Payments', path: '/pending-payments', show: user.role === 'Front Desk Executive' || user.role === 'Admin' },
    { icon: Folder, label: 'Resources', path: '/resources', show: true },
    { icon: Trash2, label: 'Deleted Leads', path: '/deleted-leads', show: user.role === 'Admin' },
